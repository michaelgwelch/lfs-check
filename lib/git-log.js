...
function processCommitChunk(commitChunk) {
  ...
  if (lines.length > 1) {
  // first line has the word commit followed by the hash
...
async function gitLogNumStat(branch, base = 'master') {
  ...
  //      hyphens (-) for binary files. The hyphens are used to find binaries with the regex
  ...
  //      following: 'releases/{ => images}/authentication-hld.jpg
  ...
  const { stdout } = await exec(`git log --numstat --no-renames --diff-filter=d --format=format:"%h %s" ${base}..${branch}`);

  // Output will be something like the following with a trailing blank line for each commit
  // commit 1ff8e4d
  // 2       2       releases/scheduler/images/schedule-service-components.png
  // 182     3       releases/scheduler/scheduler-database-design.md
  // -       -       releases/scheduler/schedule-service-database-design.xlsx
  //
